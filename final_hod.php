...
if (isset($_POST['HOD'])) {
  ...
  $sql = "SELECT * FROM `hod` WHERE id_h='$hu' && pass_h ='$hp'";
  $result = $conn->query($sql);

  if ($result->num_rows > 0) {
    $_SESSION['hod_user'] = $hu;
    $_SESSION['hod_pass'] = $hp;
  } else {
...
if (!isset($_SESSION['hod_user']) && !isset($_SESSION['hod_pass'])) {
  $_SESSION['result'] = "please login First";
  header("Location: ./home_page1.php");
}

$hu = $_SESSION['hod_user'];
$hp = $_SESSION['hod_pass'];
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
    ...
    <a href="javascript:void(0);" style="font-size:15px;" class="icon" onclick="myFunction()">&#9776;</a>
      ...
      <div class="container-fluid" id="top" style="background-color:#f6f6fa;height: 490px;">
        <div class="row">
          ...
          <div class="col-sm-8" style="padding-top:100px;"> 
            <h1 class="text-left" style="margin-top: 140px;margin-left:25px;font-size: 50px;"><b><?= $row["name_h"] ?> </b></h1>
      ...
      <table>
        ...
        $sql = "SELECT form1.sr_no, form1.staff_id,form1.name,form1.reason,form1.period_leave_req_p_s,profile.rem_casual,nature_leave,DATEDIFF(leave_till, leave_from ) AS total_days FROM `form1` INNER JOIN `profile` ON `profile`.id=`form1`.staff_id WHERE form1.status=1";
        $res = $conn->query($sql);
              ...
              <td>
                <div class="accept" style="display:flex;">
                  <form action="./approve.php" method="post">
                    ...
                    <input type="hidden" value=<?= $row1["rem_casual"] ?> name="rem_casual">
                    <input type="hidden" value=<?= $row1["total_days"] ?> name="total_days">
                    <button class="btn_lower" type="submit" style=" cursor:pointer">Approve</button>
                  </form>
                  ...
                  <form action="./deny.php" method="post">
                    ...
                    <input type="hidden" value=<?= $row1["staff_id"] ?> name="staff_id">
                    <input type="hidden" value="form1" name="table">
                    <button class="btn_lower1" type="submit" style=" cursor:pointer">Non-Approve</button>
                  </form>
                </div>
              </td>
            </tr>
        ...
        $sql = "select form2.sr_no,form2.id,form2.name,reason, DATEDIFF(leave_till, leave_from)+1 AS total_days,rem_medical,status from form2 INNER JOIN `profile` ON `profile`.id=`form2`.id WHERE form2.status=1";
        $res = $conn->query($sql);
        if ($res->num_rows > 0) {
          while ($row1 = mysqli_fetch_assoc($res)) {
          ?>
              ...
              <td>
                <div class="accept" style="display:flex;">
                  <form action="./approve.php" method="post">
                    ...
                    <input type="hidden" value="<?= $row1["name"] ?>" name="name">
                    <input type="hidden" value="<?= $row1["total_days"] ?>" name="total_days">
                    <input type="hidden" value="<?= $row1["rem_medical"] ?>" name="rem_medical">
